#Lab 12 Vasile Danciu, Josh Jones, Lavinia Uruc
#Spongebob SquarePants strategy text based game part 2
...
def help():
  ...
  printNow("Your mission is to pick a Krabby Patty from the Krusty Krab and take it to Patrick's house")
  printNow("You are going to need a spatula to pick the krabby patty!")
  printNow("To travel from one location to another,")
  printNow("type a word like 'north', 'south', 'west',or 'east' to reach a destination ")
  printNow("You can also pick objects at some locations by typing 'pick object_name' ")
...
def patrick():
  ...
  if (object2picked == true):#if player has object2picked (the patty), he wins the game
    printNow("Congratulations Spongebob, you won the game! Patrick ate the delicious krabby patty!\n")
    printNow("Game Over")
    check = false
    exit()
...
def chum():
  ...
  if (object3picked == true):# if player went to Chum Bucket with object3picked (secret formula), he loses the game
    printNow("Plankton stole your Krabby Patty Secret Formula, Spongebob and you lost the game!\n")
    printNow("Game Over")
    check = false
    exit()
...
def krusty():
  ...
  # destination choice loop
  while check == true:
    dirChc = "You are THE KRUSTY KRAB\n"
    dirChc += "Where would you like to go, Spongebob?\n"
